(()=>{var c=!1,g=null,u=!1;function m(){let e=new URL(window.location.href),t=e.pathname.match(/\/product\/(\d+)\/(\d+)/);if(t&&t.length>2){let n=t[1];return t[2]}let i=e.search.match(/-i\.(\d+)\.(\d+)/);if(i&&i.length>2){let n=i[1];return i[2]}return null}function E(){if(document.getElementById("shopee-commission-widget-icon"))return;let e=document.createElement("div");e.id="shopee-commission-widget-icon",e.innerHTML="\u{1F4B0}",e.style.cssText=`
        position: fixed;
        bottom: 60px;
        right: 20px;
        width: 50px;
    ...
    `,document.body.appendChild(e),document.getElementById("widget-close-btn").addEventListener("click",()=>{y()})}function y(){let e=document.getElementById("shopee-commission-widget-panel");e&&(c=!c,e.style.display=c?"block":"none",c&&x())}async function x(){let e=m();if(!e){document.getElementById("widget-content").innerHTML=`
        ...
        `;return}g=e,chrome.runtime.sendMessage({type:"CALCULATE_PRODUCT_STATS",productId:e},t=>{if(chrome.runtime.lastError){document.getElementById("widget-content").innerHTML=`
            ...
            `})}function T(e){var i,n,o,d,a,r;if(!e||e.totalOrders===0){document.getElementById("widget-content").innerHTML=`
            <div style="text-align: center; padding: 20px; color: #999;">
        ...
        <div style="padding: 10px; font-family: Arial, sans-serif;">
            ...
            <div style="margin-bottom: 10px;">
                ...
                <span style="font-size: 16px; font-weight: bold; color: #333;">${((i=e.formatted)==null?void 0:i.totalGMV)||"0 \u20AB"}</span>
            </div>
    ...
    `;document.getElementById("widget-content").innerHTML=t}function B(){if(document.getElementById("shopee-link-widget-icon"))return;let e=document.createElement("div");e.id="shopee-link-widget-icon",e.innerHTML="\u{1F517}",e.style.cssText=`
        ...
        opacity: 0.5;
        transition: transform 0.2s, opacity 0.2s;
    `,e.addEventListener("mouseenter",()=>{e.style.transform="scale(1.1)",e.style.opacity="1"}),e.addEventListener("mouseleave",()=>{e.style.transform="scale(1)",e.style.opacity="0.5"}),e.addEventListener("click",()=>{C()}),document.body.appendChild(e)}function z(){if(document.getElementById("shopee-link-widget-panel"))return;let e=document.createElement("div");e.id="shopee-link-widget-panel",e.style.cssText=`
        position: fixed;
        bottom: 60px;
        right: 80px;
        width: 400px;
    ...
    `,document.body.appendChild(e),document.getElementById("affiliate-link-widget-close-btn").addEventListener("click",()=>{C()});let t=document.getElementById("affiliate-link-url-input");t&&(t.value=window.location.href),S()}function S(){let e=document.getElementById("affiliate-link-create-btn");e&&e.addEventListener("click",A);let t=document.getElementById("affiliate-link-short-copy-btn");t&&t.addEventListener("click",()=>{let o=document.getElementById("affiliate-link-short-result");o&&w(o)});let i=document.getElementById("affiliate-link-long-copy-btn");i&&i.addEventListener("click",()=>{let o=document.getElementById("affiliate-link-long-result");o&&w(o)});let n=document.getElementById("affiliate-link-history-btn");n&&n.addEventListener("click",R)}async function A(){var a,r,b,h,v;let e=document.getElementById("affiliate-link-url-input"),t=document.getElementById("affiliate-link-result"),i=document.getElementById("affiliate-link-error"),n=document.getElementById("affiliate-link-create-btn");if(!e||!t||!i||!n)return;i.style.display="none",t.style.display="none";let o=e.value.trim();if(!o){p("Vui l\xF2ng nh\u1EADp URL");return}if(!o.includes("shopee.vn")){p("URL ph\u1EA3i l\xE0 trang Shopee (shopee.vn)");return}let d={subId1:((a=document.getElementById("affiliate-link-sub1"))==null?void 0:a.value.trim())||"",subId2:((r=document.getElementById("affiliate-link-sub2"))==null?void 0:r.value.trim())||"",subId3:((b=document.getElementById("affiliate-link-sub3"))==null?void 0:b.value.trim())||"",subId4:((h=document.getElementById("affiliate-link-sub4"))==null?void 0:h.value.trim())||"",subId5:((v=document.getElementById("affiliate-link-sub5"))==null?void 0:v.value.trim())||""};n.disabled=!0,n.textContent="\u0110ang t\u1EA1o...";try{chrome.runtime.sendMessage({type:"CREATE_AFFILIATE_LINK",originalLink:o,subIds:d},l=>{if(n.disabled=!1,n.textContent="T\u1EA1o link",chrome.runtime.lastError){p("L\u1ED7i: "+chrome.runtime.lastError.message);return}if(l&&l.success){let s=document.getElementById("affiliate-link-short-result"),k=document.getElementById("affiliate-link-long-result");s&&(s.value=l.shortLink||""),k&&(k.value=l.longLink||""),t.style.display="block",M({originalLink:o,subIds:d,shortLink:l.shortLink||"",longLink:l.longLink||""})}else{let s=(l==null?void 0:l.error)||"Kh\xF4ng th\u1EC3 t\u1EA1o link";s==="UNAUTHORIZED"&&(s="Vui l\xF2ng \u0111\u0103ng nh\u1EADp v\xE0o https://affiliate.shopee.vn tr\u01B0\u1EDBc"),p(s)}})}catch(l){n.disabled=!1,n.textContent="T\u1EA1o link",p("L\u1ED7i: "+l.message)}}function p(e){let t=document.getElementById("affiliate-link-error");t&&(t.textContent=e,t.style.display="block")}function w(e){var t;e.select(),e.setSelectionRange(0,99999);try{document.execCommand("copy");let i=((t=e.nextElementSibling)==null?void 0:t.textContent)||"";e.nextElementSibling&&(e.nextElementSibling.textContent="\u0110\xE3 copy!",setTimeout(()=>{e.nextElementSibling&&(e.nextElementSibling.textContent=i)},2e3))}catch(i){console.error("Copy failed:",i)}}async function M(e){try{let{affiliateLinkHistory:t=[]}=await chrome.storage.local.get("affiliateLinkHistory"),i={...e,createdAt:new Date().toLocaleString("vi-VN"),timestamp:Date.now()};t.unshift(i),t.length>100&&t.splice(100),await chrome.storage.local.set({affiliateLinkHistory:t})}catch(t){console.error("Error saving affiliate link:",t)}}async function H(){try{let{affiliateLinkHistory:e=[]}=await chrome.storage.local.get("affiliateLinkHistory");return e}catch(e){return console.error("Error loading affiliate link history:",e),[]}}async function R(){let e=document.getElementById("affiliate-link-history-list"),t=document.getElementById("affiliate-link-history-btn");if(!(!e||!t))if(e.style.display==="none"||!e.style.display){let i=await H();if(i.length===0)e.innerHTML='<div style="padding: 10px; text-align: center; color: #999; font-size: 13px;">Ch\u01B0a c\xF3 l\u1ECBch s\u1EED</div>';else{let n='<div style="max-height: 300px; overflow-y: auto;">';i.forEach((o,d)=>{n+=`
                    <div style="padding: 10px; border-bottom: 1px solid #eee; ${d===i.length-1?"border-bottom: none;":""}">
                ...
                `}),n+="</div>",e.innerHTML=n,e.querySelectorAll(".history-copy-btn").forEach(o=>{o.addEventListener("click",d=>{let a=d.target.getAttribute("data-link");if(a){let r=document.createElement("input");r.value=a,document.body.appendChild(r),r.select(),document.execCommand("copy"),document.body.removeChild(r),d.target.textContent="\u0110\xE3 copy!",setTimeout(()=>{d.target.textContent="Copy"},2e3)}})})}e.style.display="block",t.textContent="\u1EA8n l\u1ECBch s\u1EED"}else e.style.display="none",t.textContent="L\u1ECBch s\u1EED"}function C(){let e=document.getElementById("shopee-link-widget-panel");if(e&&(u=!u,e.style.display=u?"block":"none",u)){let t=document.getElementById("affiliate-link-url-input");t&&(t.value=window.location.href),document.getElementById("affiliate-link-result").style.display="none",document.getElementById("affiliate-link-history-list").style.display="none",document.getElementById("affiliate-link-error").style.display="none"}}chrome.runtime.onMessage.addListener((e,t,i)=>{if(e.type==="SHOW_PRODUCT_STATS"){let n=e.productId||m();n&&(g=n,c?x():y()),i({success:!0})}return!0});document.readyState==="loading"?document.addEventListener("DOMContentLoaded",I):I();function I(){m()&&(E(),L());let t=new URL(window.location.href);t.hostname==="shopee.vn"&&t.pathname!=="/"&&t.pathname.length>1&&(B(),z()),U()}function U(){let e=window.location.href;window.addEventListener("popstate",f);let t=history.pushState,i=history.replaceState;history.pushState=function(...o){t.apply(history,o),setTimeout(f,100)},history.replaceState=function(...o){i.apply(history,o),setTimeout(f,100)},new MutationObserver(()=>{window.location.href!==e&&(e=window.location.href,f())}).observe(document,{childList:!0,subtree:!0}),setInterval(()=>{window.location.href!==e&&(e=window.location.href,f())},1e3)}function f(){let e=new URL(window.location.href);if(e.hostname==="shopee.vn"&&e.pathname!=="/"&&e.pathname.length>1){if(document.getElementById("shopee-link-widget-icon")||B(),!document.getElementById("shopee-link-widget-panel"))z();else if(u){let i=document.getElementById("affiliate-link-url-input");i&&(i.value=window.location.href)}}else{let i=document.getElementById("shopee-link-widget-icon"),n=document.getElementById("shopee-link-widget-panel");i&&i.remove(),n&&n.remove()}let t=m();if(t)document.getElementById("shopee-commission-widget-icon")||E(),document.getElementById("shopee-commission-widget-panel")||L(),g!==t&&(g=t,c&&x());else{let i=document.getElementById("shopee-commission-widget-icon"),n=document.getElementById("shopee-commission-widget-panel");i&&i.remove(),n&&n.remove()}}})();
