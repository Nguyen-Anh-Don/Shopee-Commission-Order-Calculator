        ...
        if (!urlPattern.test(currentUrl)) {
            ...
            const usageInstructions = `
                <ol>
                    ...
                    <li><strong>Chọn ngày</strong> bạn muốn xem kết quả. <strong>Nếu không thể chọn ngày hôm qua, hãy chọn ngày hôm kia trước, sau đó nhấn "Tìm kiếm". Lúc này, hệ thống sẽ cho phép bạn chọn lại ngày hôm qua</strong>.</li>
                ...
                <div class="container mt-2">
                    ...
                    <p class="mb-1"><strong>VIP: <a href="https://goink.me/9enf" target="_blank">Lọc theo SubID</a></strong></p>
                </div>
            `;
...
async function calculateAllPages() {
    ...
    function calculateCurrentPage() {
        ...
        orderRows.forEach((row) => {
            ...
            const gmvEl = row.querySelector(".commission-wrap .commission-top span");
            ...
            if (gmvEl) {
                ...
                if (!isNaN(gmv)) {
                    totalGMV += gmv;
                    console.log("GMV found:", gmv, "Total GMV:", totalGMV); // Thêm log để debug
                }
            }

            // Nếu các selector đúng thì lấy
            if (shopNameEl && productNameEl && commissionEl) {
                const commission = parseFloat(commissionEl.textContent.replace(/[₫,.]/g, "").replace(/,/g, "."));

                // Phân loại hoa hồng theo loại đơn
                if (referrer.includes("Shopeevideo")) {
                    videoCommission += commission;
                } else if (referrer.includes("Shopeelive")) {
                    liveCommission += commission;
                } else {
                    socialCommission += commission;
                }

                // Nếu đơn bị hủy, thêm vào hoa hồng hủy
                if (isCanceled) {
                    canceledCommission += commission;
                }

                // Nếu hoa hồng bằng 0, thêm vào hoa hồng 0đ
                if (commission === 0) {
                    zeroCommission += commission;
                }
                            ...
                            items: [
                                {
                                    ...
                                    item_id: itemId,
                                    referrer: referrer,
                                    item_gmv: gmv, // Thêm GMV vào dữ liệu
                                },
                            ],
                        },
                    ],
    ...
    async function processNextPage() {
        ...
        if (nextPageButton && !nextPageButton.classList.contains("ant-pagination-disabled")) {
            ...
            return {
                ...
                totalOrders: `${totalOrders - canceledOrders / 2 - unpaidOrders / 2} (${totalOrders} - <span class="badge bg-secondary">${unpaidOrders / 2}</span> - <span class="badge bg-danger">${
                    canceledOrders / 2
                }</span>)`,
                ...
                // Thêm hoa hồng theo loại đơn vào kết quả
